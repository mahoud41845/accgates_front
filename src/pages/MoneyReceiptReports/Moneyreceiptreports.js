...
import "./moneyreceiptreports.css";
import {
  faFileExcel,
  faFileInvoice,
  faFileInvoiceDollar,
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post(
        "http://127.0.0.1:8000/api/receiveMoney",
        formData,
        {
          headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get("http://127.0.0.1:8000/api/receiveMoney", {
          headers: { Authorization: `Bearer ${token}` },
        });
          ...
          <div className="Moneyreceiptreports-mainform">
            <div className="Moneyreceiptreports-formbody">
              ...
              <SelectComponent
                ...
                onChange={(value) => {
                  console.log("Selected Payment Account ID:", value);
              ...
              <SelectComponent
                ...
                onChange={(value) => {
                  console.log("Selected Receiver Account ID:", value);
                  setFormData({ ...formData, receiver_account_id: value });
                }}
              />
            </div>
...
export default Moneyreceiptreports;
