  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://127.0.0.1:8000/api/purchase-invoice",
        invoiceData,
        {
          headers: {
            "Content-Type": "application/json",
      ...
      console.error("Error:", error);
      alert(error.response || "An error occurred while creating the invoice.");
    }
  };
  ...
  axios
    ...
    .then((response) => {
      if (response.data && response.data.data) {
        ...
        setItems([]); // إذا لم تكن هناك بيانات، ضع مصفوفة فارغة لتجنب الخطأ
        console.error("Error: Received empty or invalid data from API");
      }
    })
    .catch((error) => {
      console.error("Error fetching items:", error);
      setItems([]); // وضع مصفوفة فارغة عند حدوث خطأ لتجنب الأخطاء في الـ map
    });
}, []);
  ...
  if (item) {
    setUnits(
      (item.unit_items || []).map((unitItem) => ({
        ...
        price: unitItem.retail_selling_price,
      }))
    );

    if (item.unit_items && item.unit_items.length > 0) {
    ...
    setUnits([]); // تجنب الأخطاء عند عدم وجود وحدات
